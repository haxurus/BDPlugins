...
module.exports = class Lockscreen {
    ...
    start() {
        ...
        if (this.hasCredential() && (this.settings.lockOnStartup || wasLocked)) {
            ...
            this.api.UI.showToast("Haxurus Lockscreen: configura un metodo di sblocco dalle impostazioni del plugin.", {type: "warning", timeout: 7000});
        }
    }
    ...
    lock() {
        ...
        if (!this.hasCredential()) {
            this.api.UI.showToast("Configura prima un codice, una password o un segno.", {type: "warning"});
            return;
        }
    ...
    createOverlay() {
        ...
        overlay.setAttribute("aria-label", "Discord bloccato");
        ...
        title.textContent = "Discord bloccato";
        ...
        footer.textContent = this.settings.enableShortcut ? "Blocca rapidamente con Ctrl + Shift + L" : "Haxurus Lockscreen";
        ...
        const updateClock = () => {
            const now = new Date();
            time.textContent = now.toLocaleTimeString("it-IT", {hour: "2-digit", minute: "2-digit"});
    ...
    getMethodPrompt() {
        switch (this.settings.method) {
            case "pin4": return "Inserisci il PIN di 4 cifre";
            case "pin6": return "Inserisci il PIN di 6 cifre";
            case "password": return "Inserisci la password";
            case "pattern": return "Disegna il segno per sbloccare";
            default: return "Inserisci le credenziali";
        }
    }
    ...
    renderPinUnlock(container, errorElement) {
        ...
        for (const key of keys) {
            ...
            if (key === "clear") {
                ...
                button.setAttribute("aria-label", "Cancella PIN");
            }
            else if (key === "back") {
                button.classList.add("hls-key-small");
                button.textContent = "⌫";
                button.setAttribute("aria-label", "Cancella ultima cifra");
            }
            else {
                button.textContent = key;
                button.setAttribute("aria-label", `Cifra ${key}`);
            }
    ...
    renderPasswordUnlock(container, errorElement) {
        ...
        input.setAttribute("aria-label", "Password di sblocco");
    ...
    renderPatternUnlock(container, errorElement) {
        ...
        hint.textContent = "Collega almeno 4 punti";

        const pattern = this.createPatternPad({
            onComplete: async sequence => {
                if (this.isLockedOut()) return false;
                if (sequence.length < 4) {
                    errorElement.textContent = "Il segno deve contenere almeno 4 punti.";
                    return false;
                }
    ...
    createPatternPad({onComplete, disabled = () => false, compact = false} = {}) {
        ...
        pad.setAttribute("aria-label", "Area per disegnare il segno");
    ...
    registerFailure(errorElement, shakeElement) {
        ...
        if (this.failedAttempts >= 5) {
            ...
            if (lockoutSeconds > 0) {
                ...
                this.lockoutUntil = 0;
                errorElement.textContent = "Credenziale errata. Puoi riprovare.";
            }
        }
        else {
            const remaining = 5 - this.failedAttempts;
            errorElement.textContent = `Credenziale errata. ${remaining} ${remaining === 1 ? "tentativo" : "tentativi"} prima del blocco temporaneo.`;
        }
    }
    ...
    startLockoutCountdown(errorElement) {
        ...
        const update = () => {
            ...
            if (!seconds) {
                clearInterval(this.lockoutTimer);
                this.lockoutTimer = null;
                errorElement.textContent = "Puoi riprovare.";
                return;
            }
            errorElement.textContent = `Troppi tentativi. Riprova tra ${seconds} ${seconds === 1 ? "secondo" : "secondi"}.`;
        };
        update();
    ...
    updateLockButton() {
        ...
        button.type = "button";
        button.title = "Blocca Discord";
        button.setAttribute("aria-label", "Blocca Discord");
        button.innerHTML = `
            <svg viewBox="0 0 24 24" aria-hidden="true">
    ...
    getSettingsPanel() {
        ...
        heading.innerHTML = `
            ...
            <div>
                ...
                <p>Protezione locale dell'interfaccia Discord.</p>
            </div>`;

        const warning = document.createElement("div");
        warning.className = "hls-settings-warning";
        warning.innerHTML = "<strong>Nota di sicurezza:</strong> questa è una protezione visiva locale. Chi può disabilitare BetterDiscord, rimuovere il plugin o modificare i suoi dati può aggirarla.";

        const credentialCard = this.createSettingsCard("Metodo di sblocco", "Imposta o sostituisci la credenziale usata dalla schermata di blocco.");
        const methodRow = this.createSettingRow("Tipo", "Scegli PIN, password o segno.");
        const methodSelect = document.createElement("select");
        methodSelect.className = "hls-settings-input";
        methodSelect.innerHTML = `
            <option value="pin4">PIN di 4 cifre</option>
            <option value="pin6">PIN di 6 cifre</option>
            <option value="password">Password</option>
            <option value="pattern">Segno</option>`;
        ...
        const renderCredentialSetup = () => {
            ...
            if (method === "pattern") {
                ...
                instructions.textContent = "Disegna il nuovo segno";

                const pad = this.createPatternPad({
                    compact: true,
                    onComplete: sequence => {
                        if (sequence.length < 4) {
                            status.textContent = "Il segno deve contenere almeno 4 punti.";
                        ...
                        if (stage === 1) {
                            ...
                            stage = 2;
                            instructions.textContent = "Ripeti il segno per confermare";
                            status.textContent = "Primo segno registrato.";
                        ...
                        if (firstPattern !== confirmedPattern) {
                            ...
                            stage = 1;
                            instructions.textContent = "I segni non coincidono. Ricomincia";
                            status.textContent = "Conferma non riuscita.";
                            status.className = "hls-settings-status error";
                            return false;
                        }

                        instructions.textContent = "Segno confermato";
                        status.textContent = "Premi Salva segno per applicarlo.";
                        status.className = "hls-settings-status success";
                        return true;
                    }
                });
                ...
                const reset = this.createButton("Ricomincia", "secondary", () => {
                    firstPattern = null;
                    confirmedPattern = null;
                    stage = 1;
                    instructions.textContent = "Disegna il nuovo segno";
                ...
                const save = this.createButton("Salva segno", "primary", async () => {
                    if (!firstPattern || !confirmedPattern || firstPattern !== confirmedPattern) {
                        status.textContent = "Disegna e conferma prima il segno.";
                        status.className = "hls-settings-status error";
                        return;
                    }
                    save.disabled = true;
                    ...
                    try {
                        ...
                        status.textContent = "Segno aggiornato correttamente.";
                        status.className = "hls-settings-status success";
                        this.updateLockButton();
                    }
                    catch (error) {
                        this.api.Logger.error("Unable to save pattern", error);
                        status.textContent = "Impossibile salvare il segno.";
                        status.className = "hls-settings-status error";
                    }
                    save.disabled = false;
                });
            ...
            const firstRow = this.createSettingRow("Nuova credenziale", requiredLength ? `Inserisci esattamente ${requiredLength} cifre.` : "Usa almeno 4 caratteri.");
            const firstInput = document.createElement("input");
            firstInput.className = "hls-settings-input";
            firstInput.type = "password";
            firstInput.autocomplete = "new-password";
            firstInput.placeholder = requiredLength ? "•".repeat(requiredLength) : "Nuova password";
            if (requiredLength) {
            ...
            const confirmRow = this.createSettingRow("Conferma", "Ripeti la stessa credenziale.");
            const confirmInput = document.createElement("input");
            confirmInput.className = "hls-settings-input";
            confirmInput.type = "password";
            confirmInput.autocomplete = "new-password";
            confirmInput.placeholder = "Conferma";
            if (requiredLength) {
            ...
            const save = this.createButton("Salva credenziale", "primary", async () => {
                const first = firstInput.value;
                const second = confirmInput.value;
                status.className = "hls-settings-status";

                if (requiredLength && !new RegExp(`^\\d{${requiredLength}}$`).test(first)) {
                    status.textContent = `Il PIN deve contenere esattamente ${requiredLength} cifre.`;
                    status.classList.add("error");
                    return;
                }
                if (!requiredLength && first.length < 4) {
                    status.textContent = "La password deve contenere almeno 4 caratteri.";
                    status.classList.add("error");
                    return;
                }
                if (first !== second) {
                    status.textContent = "Le credenziali non coincidono.";
                    status.classList.add("error");
                    return;
                }
                ...
                try {
                    ...
                    firstInput.value = "";
                    confirmInput.value = "";
                    status.textContent = "Credenziale aggiornata correttamente.";
                    status.className = "hls-settings-status success";
                    this.updateLockButton();
                }
                catch (error) {
                    this.api.Logger.error("Unable to save credential", error);
                    status.textContent = "Impossibile salvare la credenziale.";
                    status.className = "hls-settings-status error";
                }
                save.disabled = false;
            });
        ...
        const behaviorCard = this.createSettingsCard("Comportamento", "Scegli quando Discord deve bloccarsi.");
        behaviorCard.body.append(
            this.createToggleRow("Blocca all'avvio", "Mostra la lockscreen quando Discord viene avviato.", "lockOnStartup"),
            this.createNumberRow("Blocco per inattività", "Minuti senza attività prima del blocco. Usa 0 per disattivarlo.", "inactivityMinutes", 0, 1440),
            this.createNumberRow("Durata del lockout", "Secondi di attesa dopo 5 tentativi errati. Usa 0 per disattivarlo.", "lockoutSeconds", 0, 3600),
            this.createToggleRow("Mostra pulsante di blocco", "Aggiunge un piccolo pulsante nell'angolo inferiore destro.", "showLockButton", () => this.updateLockButton()),
            this.createToggleRow("Scorciatoia Ctrl + Shift + L", "Permette di bloccare Discord dalla tastiera.", "enableShortcut")
        );

        const actionsCard = this.createSettingsCard("Azioni", "Gestione immediata della lockscreen.");
        const actionButtons = document.createElement("div");
        actionButtons.className = "hls-settings-actions hls-settings-actions-wide";

        const lockNow = this.createButton("Blocca ora", "primary", () => {
            if (!this.hasCredential()) {
                this.api.UI.showToast("Configura prima una credenziale.", {type: "warning"});
                return;
            }
            this.lock();
        });

        const removeCredential = this.createButton("Rimuovi credenziale", "danger", () => {
            this.api.UI.showConfirmationModal(
                "Rimuovere la credenziale?",
                "La lockscreen verrà disattivata finché non imposterai un nuovo metodo di sblocco.",
                {
                    danger: true,
                    confirmText: "Rimuovi",
                    cancelText: "Annulla",
                    onConfirm: () => {
                        ...
                        this.api.Data.save("locked", false);
                        this.updateLockButton();
                        this.api.UI.showToast("Credenziale rimossa.", {type: "success"});
                    }
                }
            );
        });
